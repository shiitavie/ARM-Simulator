...
char stall_flag = 0;
int count = 1;
char data_miss_flag = 0;
char instruction_miss_flag = 0;
int instruction_buffer = 0;
...
int curr_inst_set_index, curr_inst_way_index, curr_data_set_index, curr_data_way_index;

//simpler cache read and write

int inst_cache_read(uint64_t addr) {
    return cache_read(instruction_cache,addr,count, curr_inst_set_index, curr_inst_way_index);
}

int data_cache_read(uint64_t addr) {
    return cache_read(data_cache,addr,count, curr_data_set_index, curr_data_way_index);
}
void data_cache_write(uint64_t addr,uint32_t value) {
    return cache_write(data_cache,addr,count, curr_data_set_index, curr_data_way_index, value);
}
...
void pipe_stage_wb()
{
    ...
    if (instruction_miss_flag) {
        instruction_miss_flag -= 1;
    }
    if (data_miss_flag || (instruction_miss_flag && (instruction_miss_flag < 48))) {
        return;
    }
    printf("### WB: \n");
...
void pipe_stage_mem()
{
    ...
    if ((instruction_miss_flag && (instruction_miss_flag < 49)) || (data_miss_flag > 1)) {
        return;
    } else if (data_miss_flag == 1) {
        cache_update(data_cache, EXtoMEM.address, count, curr_data_set_index, curr_data_way_index);
        //data_miss_flag -= 1;
        EXtoMEM.void_flag = 0; // in cache returns, start excuteing? questionableeee
    }
    ...
    if (EXtoMEM.mem_flag){
        ...
        if (EXtoMEM.write_flag) {
            if (!hit_or_miss(data_cache, EXtoMEM.address, &curr_data_set_index, &curr_data_way_index)) {
                data_miss_flag = 51;
                return;
            }
            // Save old contents of the address
            uint64_t old_contents = data_cache_read(EXtoMEM.address);
            printf("Remainder: %ld\n", EXtoMEM.address % 4);
            switch (EXtoMEM.byte_count) {
                ...
                case(8):
                    ...
                    data_cache_write(EXtoMEM.address, EXtoMEM.value & 0xFFFFFFFF);
                    data_cache_write(EXtoMEM.address + 4, (EXtoMEM.value & 0xFFFFFFFF00000000) >> 32);
                    break;
                case(4):
                    printf("%s", "Stur4\n");
                    data_cache_write(EXtoMEM.address, EXtoMEM.value);
                    break;
                case(2):
                    printf("%s", "Stur2\n");
                    ...
                    {
                        EXtoMEM.address -= 1;
                        old_contents = data_cache_read(EXtoMEM.address);
            ...
            MEMtoWB.value = data_cache_read(EXtoMEM.address);
            uint64_t value2 = data_cache_read(EXtoMEM.address + 4); //when is this needed?
            ...
            switch (EXtoMEM.byte_count) {
                ...
                case(2):
                    ...
                    {
                        EXtoMEM.address -= 1;
                        MEMtoWB.value = (data_cache_read(EXtoMEM.address) & 0xffff00) >> 8;
                    }
                    else if(EXtoMEM.address % 4 == 2)
                    {
                        EXtoMEM.address -= 2;
                        MEMtoWB.value = (data_cache_read(EXtoMEM.address) & 0xffff0000) >> 16;
                    }
                    else if(EXtoMEM.address % 4 == 3)
                    {
                        EXtoMEM.address -= 3;
                        MEMtoWB.value = (data_cache_read(EXtoMEM.address) & 0xff000000) >> 24;
                        EXtoMEM.address += 4;
                        MEMtoWB.value = MEMtoWB.value | (data_cache_read(EXtoMEM.address & 0xff) << 8);
                    }
                    else
                    {
                ...
                case(1):
                    ...
                    {
                        EXtoMEM.address -= 1;
                        MEMtoWB.value = (data_cache_read(EXtoMEM.address) & 0xff00) >> 8;
                        printf("Case 1");
                    }
                    else if(EXtoMEM.address % 4 == 2)
                    {
                        EXtoMEM.address -= 2;
                        MEMtoWB.value = (data_cache_read(EXtoMEM.address) & 0x00ff0000) >> 16;
                        printf("Case 2");
                    }
                    else if(EXtoMEM.address % 4 == 3)
                    {
...
void pipe_stage_execute()
{
    ...
    if ((instruction_miss_flag && (instruction_miss_flag < 50)) || (data_miss_flag > 1)){
        return;
    }
    if (!DEtoEX.ex_flag)
    {
...
{
    printf("###Decode \n");
    if ((instruction_miss_flag && (instruction_miss_flag < 51)) || (data_miss_flag > 1)  //&& data_miss_flag <51
        || stall_flag || (!IFtoDE.de_flag || !if_flag)) {
        return;
    }
    if (IFtoDE.void_flag) {
        DEtoEX.void_flag = IFtoDE.void_flag;
        IFtoDE.void_flag -= 1;
    }
...
{
    printf("###Fetch \n");
    if (instruction_miss_flag == 50)
    {
        if (hit_or_miss(instruction_cache, CURRENT_STATE.PC, &curr_inst_set_index, &curr_inst_way_index))
        {
            instruction_miss_flag = 0;
        }
    }
    printf("Instruction miss: %d \n", instruction_miss_flag);
    printf("Data miss: %d \n", data_miss_flag);

    if (instruction_miss_flag == 1) {
        cache_update(instruction_cache, CURRENT_STATE.PC, count, curr_inst_set_index, curr_inst_way_index);
        if (data_miss_flag == 2) {
            IFtoDE.void_flag = 1;
        } else if (data_miss_flag == 3) {
            IFtoDE.void_flag = 1;
            DEtoEX.void_flag = 2;
        } else if (data_miss_flag == 4) {
            IFtoDE.void_flag = 1;
            DEtoEX.void_flag = 2;
            EXtoMEM.void_flag = 3;
        } else if (data_miss_flag == 1) {
    ...
    if ((instruction_miss_flag > 1)|| (data_miss_flag > 1 && data_miss_flag < 51) || stall_flag || !if_flag || (MEMtoWB.hlt_flag || EXtoMEM.hlt_flag)) {
    ...
    if (instruction_buffer) {
        IFtoDE.instruction = instruction_buffer;
        instruction_buffer = 0;
        CURRENT_STATE.PC -= 4;
        printf("instruction buffer emptied and instruction is %x \n", IFtoDE.instruction);
    } else if (hit_or_miss(instruction_cache, CURRENT_STATE.PC, &curr_inst_set_index, &curr_inst_way_index)) {
        printf("Hit or miss successful\n");
        if (data_miss_flag == 51) {
            instruction_buffer = inst_cache_read(CURRENT_STATE.PC);
        } else {
            IFtoDE.instruction = inst_cache_read(CURRENT_STATE.PC);
        }
        printf("Cache read successful and instruction is %x \n", IFtoDE.instruction);
    } else {
        instruction_miss_flag = 51;
